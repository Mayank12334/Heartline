const API = "http://3.138.42.18:5000";
    ...
    function renderAvatarPicker() {
      ...
      if (selectedAvatarPreview) {
        selectedAvatarPreview.innerHTML = `<img src="${avatarDataUri(selectedAvatarId)}" alt="Selected avatar">`;
      }
    }
    ...
    async function signup() {
      ...
      if (!name) {
        setValidationMessage("Enter your full name.");
        return;
      }
      ...
      try {
        const res = await fetch(`${API}/api/register`, {
          ...
          body: JSON.stringify({ name, email, password, avatarId, accountType })
        });
    ...
    document.querySelectorAll('input[name="accountType"]').forEach(input => {
      input.addEventListener("change", syncAccountTypeSelection);
    });
