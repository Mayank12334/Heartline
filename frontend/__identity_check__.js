    ...
    function openIdentityModal(userId) {
      ...
      identitySummary.textContent = identityUser.isAdmin
        ? "Founder, creator, and full-access administrator of the Hearthline experience."
        : identityUser.accountType === "private"
          ? "This writer publishes inside a private circle. Their writing opens to followers."
          : "This writer publishes openly so anyone on Hearthline can discover their work.";
    ...
    function renderProfileCard() {
      ...
      headerUserType.textContent = currentUser.isAdmin
        ? "Royal admin access"
        : `${currentUser.accountType === "private" ? "Private" : "Public"} account`;
        ...
        creatorsList.innerHTML = visibleUsers.map(user => `
          <div class="creator-card">
            <div>
              ...
              <span>${user.accountType} account • ${user.followersCount} follower${user.followersCount === 1 ? "" : "s"}</span>
            </div>
            ${token ? `
              <button
                ...
                <div class="post-topline">
                  ...
                  <div class="author-actions">
                    <button class="author-link" type="button" onclick="openIdentityModal('${post.userId?._id}')">
                      ...
                      <span class="post-author">By ${post.userId?.name || "Unknown Author"}</span>
                    </button>
                    ${followButton}
                  </div>
                </div>
